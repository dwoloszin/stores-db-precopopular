"""
scraper_drogarialecer.py — Scraper for Drogaria Lecer (https://www.drogarialecer.com.br)

Platform  : VTEX IO (account `drogarialecer`) — discount/generics chain.
...
Category note:
    fq filtering works. ~229 leaf categories, ~8.3k products, 240 leaves, NONE over the 2550
    cap (biggest leaf ~1415, verified 2026-08-25) -> plain Drogal-style leaf walk,
    no price-range subdivision needed.

Usage:
    python -m markets.drogarialecer.scraper_drogarialecer              # scrape -> DB
    python -m markets.drogarialecer.scraper_drogarialecer --limit 500  # test run -> DB
    python -m markets.drogarialecer.scraper_drogarialecer --csv        # scrape -> DB + CSV
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Scrape Drogaria Lecer -> PostgreSQL (DB is always written; CSV is optional)"
    )
    ...
    from db.db_manager import DrogariaLecerDB, load_env
    load_env(args.env)
