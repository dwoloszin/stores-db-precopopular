"""
scraper_precopopular.py — Scraper for Preço Popular (https://www.precopopular.com.br)

Platform  : VTEX IO (account `precopopular`) — discount/generics chain.
...
Category note:
    fq filtering works. ~229 leaf categories, ~19k products, 404 leaves, NONE over the 2550
    cap (biggest leaf ~1415, verified 2026-08-25) -> plain Drogal-style leaf walk,
    no price-range subdivision needed.

Usage:
    python -m markets.precopopular.scraper_precopopular              # scrape -> DB
    python -m markets.precopopular.scraper_precopopular --limit 500  # test run -> DB
    python -m markets.precopopular.scraper_precopopular --csv        # scrape -> DB + CSV
"""
...
BASE_URL  = "https://www.precopopular.com.br"                 # storefront (product_url); Cloudflare-WAF blocks datacenter IPs
API_HOST  = "https://precopopular.vtexcommercestable.com.br"  # VTEX backend host — bypasses the storefront WAF (used for all catalog API calls)
...
def fetch_category_nodes(session: requests.Session) -> List[Dict]:
    ...
    Returns a flat list of ALL leaf nodes, each with:
      id, name, fq (e.g. 'C:/1/50/726/'), full_path (human label), is_leaf=True
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Scrape Preco Popular -> PostgreSQL (DB is always written; CSV is optional)"
    )
    ...
    from db.db_manager import PrecoPopularDB, load_env
    load_env(args.env)
